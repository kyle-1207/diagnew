...
def train_transformer_with_hybrid_feedback(config, train_data, save_dir):
    ...
    for epoch in range(epochs):
        ...
        if epoch >= feedback_config['min_epochs_before_feedback']:
            ...
            # 判断是否触发反馈（支持分级阈值）
            should_feedback, trigger_reason, feedback_intensity = should_trigger_feedback(
                epoch, 
                last_feedback_epoch, 
                feedback_metrics, 
                feedback_config
            ...
            if should_feedback:
                ...
                feedback_count += 1
                last_feedback_epoch = epoch
                
                print(f"🔄 Epoch {epoch}: 触发反馈 - {trigger_reason}")
                print(f"   📊 假阳性率: {feedback_metrics['false_positive_rate']:.1%}")
                print(f"   🎯 反馈强度: {feedback_intensity:.1f}")
                
                # 执行反馈步骤（传入反馈强度）
                feedback_loss = execute_feedback_step(
                    model, 
                    feedback_config, 
                    feedback_metrics,
                    device,
                ...
                if feedback_loss > 0:
                    total_loss += feedback_loss
                    epoch_feedback_loss = feedback_loss.item()
                    
                    print(f"   💡 反馈损失: {epoch_feedback_loss:.6f}")
        ...
        feedback_losses.append(epoch_feedback_loss)
        
        # 记录反馈历史
        feedback_metrics_history.append(feedback_metrics if epoch >= feedback_config['min_epochs_before_feedback'] else {})
        feedback_trigger_history.append({
            'triggered': feedback_triggered,
        ...
        if (epoch + 1) % 20 == 0:
            ...
            print(f"   - 训练损失: {epoch_loss:.6f}")
            print(f"   - 反馈损失: {epoch_feedback_loss:.6f}")
            print(f"   - 反馈次数: {feedback_count}")
            
            if epoch >= feedback_config['min_epochs_before_feedback']:
                print(f"   - 当前假阳性率: {feedback_metrics['false_positive_rate']:.4f}")
    ...
    history = {
        ...
        'feedback_metrics_history': feedback_metrics_history,
        'feedback_trigger_history': feedback_trigger_history,
    ...
    print(f"📊 训练统计:")
    print(f"   - 总反馈次数: {feedback_count}")
    print(f"   - 最终假阳性率: {feedback_metrics_history[-1].get('false_positive_rate', 0):.4f}")
