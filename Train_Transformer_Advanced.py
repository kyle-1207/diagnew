...
def main():
    ...
    try:
        ...
        # 创建分布式数据加载器
        batch_size = 2048 * world_size  # 总批次大小
        ...
        if is_main_process():
            ...
            print(f"   等效批次大小: {batch_size * gradient_accumulation_steps}")
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                epoch_loss += reduced_loss.item() * gradient_accumulation_steps
                batch_count += 1
                
                if is_main_process():
                    current_lr = scheduler.get_last_lr()[0]
                    pbar.set_postfix({
                        'loss': f"{reduced_loss.item() * gradient_accumulation_steps:.6f}",
                        'lr': f"{current_lr:.6f}"
                    })
        ...
        if is_main_process():
            ...
            lrs = [lr_lambda(step) * learning_rate for step in steps]
